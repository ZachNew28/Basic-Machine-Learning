...
template <typename K, typename V, typename C>
std::pair<typename Map<K, V, C>::Iterator, 
          bool> Map<K, V, C>::insert(const Pair_type &val) {
  std::pair<Iterator, bool> pair;
  Iterator search = tree.find(val);
  
  if (search == end()) { //if the key is not on the map, add the element
    tree.insert(val);
    search = tree.find(val);
    pair.first = search;
    pair.second = true;
  }
  else { //if it is there, return the iterator pointing to it
    pair.first = search;
    pair.second = false;
  }
