...
// ---------- DO NOT CHANGE ANYTHING IN THIS FILE ABOVE THIS LINE ----------
  ...
  static Node *copy_nodes_impl(Node *node) {
    ...
    else { //copy datum then copy other data, once it's done it returns
  ...
  // NOTE:    This function must be tree recursive.
  //THIS PROBABLY WORKS DONT CHANGE UNLESS CONFIDENT
  static void destroy_nodes_impl(Node *node) {
    if (node == nullptr) { //empty tree
      return;
    }
    else { //go as far down as possible then destroy back to root
  ...
  static Node * find_impl(Node *node, const T &query, Compare less) {
    ...
    else if (!less(node->datum, query) && !less(query, node->datum)) { //query found
      return node;
    }
    else if (less(query, node->datum)) { //less than root (left side)
      return find_impl(node->left, query, less);
    }
    else { //greater than root (right side)
  ...
  static Node * insert_impl(Node *node, const T &item, Compare less) {
    ...
    if (less(item, node->datum)) { //left-side
      node->left = insert_impl(node->left, item, less);
    }
    else if (less(node->datum, item)) { //right-side
  ...
  static Node * min_element_impl(Node *node) {
    ...
    if (node->left == nullptr) { //if there isnt a smaller node, return this one
      return node;
    }
    else {
      node = min_element_impl(node->left);
    }
  ...
  static Node * max_element_impl(Node *node) {
    ...
    if (node->right == nullptr) { //if there isnt a bigger node, return this one
      return node;
    }
    else {
      node = max_element_impl(node->right);
    }
  ...
  static bool check_sorting_invariant_impl(const Node *node, Compare less) {
    ...
    if (node->left != nullptr && less(node->datum, max_element_impl(node->left)->datum)) {
      return false;
    }
    ...
    if (!check_sorting_invariant_impl(node->left, less) 
        || !check_sorting_invariant_impl(node->right, less)) {//Recursion
          return false;
    }
  ...
  static Node * min_greater_than_impl(Node *node, const T &val, Compare less) {
    if (node == nullptr) {
      return nullptr;
            ...
            && !less(node->datum, val))) { //Recurse down right
      node = min_greater_than_impl(node->right, val, less);
    }
    else if (less(val, node->datum)) { //Recurse down left
      Node * left_check = min_greater_than_impl(node->left, val, less);
      if (!left_check) { //return original num
        return node;
      }
